...
export const MCPConnectorsModal: React.FC<MCPConnectorsModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (activeTab === 'discover' && discoverConnectors.length === 0) {
      ...
      // Simulate API call to fetch available connectors
        ...
        <div className="px-6 py-5 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/50">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500 font-medium mt-0.5">Model Context Protocol • Gestiona las herramientas del enjambre</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="px-6 py-4 border-b border-zinc-100 flex items-center justify-between">
          <div className="flex items-center gap-1 bg-zinc-100/80 p-1 rounded-xl border border-zinc-200/50">
            <button 
              ...
              className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all ${activeTab === 'installed' ? 'bg-white text-zinc-900 shadow-sm' : 'text-zinc-500 hover:text-zinc-700'}`}
            >
              Instalados (5)
            </button>
            <button 
              onClick={() => setActiveTab('discover')}
              className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all ${activeTab === 'discover' ? 'bg-white text-zinc-900 shadow-sm' : 'text-zinc-500 hover:text-zinc-700'}`}
            >
              Descubrir
            </button>
          </div>
              ...
              {connectors.map((connector) => (
                ...
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center border ${
                    connector.type === 'custom' ? 'bg-zinc-900 border-zinc-800 text-[#D4AF37]' : 'bg-zinc-50 border-zinc-200 text-zinc-700'
                  }`}>
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="relative mb-6">
                ...
                <input 
                  ...
                  className="w-full bg-white border border-zinc-200 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#D4AF37]/50 focus:border-[#D4AF37] transition-all"
                />
              </div>
                    ...
                    <div key={connector.id} className="bg-white border border-zinc-200/80 rounded-2xl p-4 flex items-center justify-between hover:border-zinc-300 transition-colors shadow-sm">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <button 
                          ...
                          disabled={installing[connector.id] || installed[connector.id]}
